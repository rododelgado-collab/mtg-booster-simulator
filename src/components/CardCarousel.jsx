...
export const CardCarousel = ({ cards, carouselRef, highlightedCardId, mostExpensiveCardId }) => {
  return (
    <div ref={carouselRef} className="flex overflow-x-auto gap-4 px-10 pb-6 snap-x scroll-smooth perspective-[1000px]">
      {cards.map((card, i) => (
        <Card 
          key={`${card.id}-${i}`} 
          card={card} 
          ...
          index={i} // <-- ¡NUEVO! Le pasamos la posición de la carta
        />
      ))}
    </div>
  );
